...
static void InitDisplay(void)
{
    ...
    ySpace = screenHeight - InchesX(TopMargin + BottomMargin)
             - InchesX(ConsoleHeight + WindowSep);
    xScale = yScale = 1.0;
    ...
    graphicsWindow =
      CreateWindow(GWClassName, windowTitle, style,
                   graphicsRect.left, graphicsRect.top,
                   RectWidth(&graphicsRect), RectHeight(&graphicsRect),
                   consoleWindow, (HMENU) NULL, (HINSTANCE) NULL,
                   (LPSTR) NULL);
